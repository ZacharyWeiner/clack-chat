    ...
    >
      ...
      >
        <div class="p-4 md:p-12 text-center lg:text-left">
          ...
          <h1 class="text-3xl font-bold pt-8 lg:pt-0">{{ displayName }}</h1>
          <div
...
export default {
  async setup() {
    ...
    let network = window.localStorage.getItem(Constants.NETWORK);
    let chain = window.localStorage.getItem(Constants.CHAIN);
    const computer = new Computer({
      chain: chain,
      network: network,
      seed: seed
    });
    let publicKey = (await computer.db.wallet.getPublicKey()).toString();
    ...
    return { computer, publicKey, address, displayName };
  }
};
</script>
